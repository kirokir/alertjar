...
<!--
  ...
  This file is 100% complete with working Google API integration
-->
<meta charset="UTF-8">
...
<!-- --- CRITICAL FIX ---
    1. The script loading order is changed to ensure Google APIs load first.
    2. 'async' and 'defer' are removed to force a strict, sequential loading order.
       This prevents race conditions and conflicts with other libraries like Chart.js.
--- -->
...
.time-block-table tr.done { background-color: rgba(16, 185, 129, 0.05); }
.time-block-table tr.done td:not(.check-cell) { text-decoration: line-through; opacity: 0.7; }
.time-block-table tr.done .check-cell button { border-color: var(--success); background-color: var(--success); }
.time-block-table tr.done .check-cell button svg { stroke: white; }
...
.settings-row input[type="checkbox"] {
    appearance: none; width: 44px; height: 24px; background: var(--border); border-radius: 12px;
...
.settings-row input[type="checkbox"]:checked { background-color: var(--success); }
...
.toast button { margin-left: 1rem; background: rgba(255,255,255,0.2); color: white; border: 1px solid white; }
.toast button:hover { background: rgba(255,255,255,0.4); }
...
<main id="main-content" class="container">
  ...
  <div id="calendar-tab" class="tab-content"><div class="card"><div class="calendar-header"><h2 id="calendar-month"></h2><div class="calendar-nav"><button id="prev-month" class="btn btn-secondary btn-small" aria-label="Previous month">‹</button><button id="today-btn" class="btn btn-secondary btn-small">Today</button><button id="next-month" class="btn btn-secondary btn-small" aria-label="Next month">›</button></div></div><div class="calendar"><div class="calendar-grid" id="calendar-grid" role="grid"></div></div></div></div>
  ...
  <div id="settings-tab" class="tab-content">
    <div id="profile-section" class="card"><h2>User Profile</h2><div id="google-sign-in-prompt"><p>Connect your Google Account to sync data across devices and unlock cloud features.</p><button id="google-connect-btn" class="btn btn-primary">Connect with Google</button></div><div id="user-profile-card" class="profile-card hidden"><img id="user-avatar" class="profile-avatar" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" alt="User Avatar"><div><div class="profile-info"><h3 id="user-name"></h3><p id="user-email"></p><p>Member since: <span id="user-join-date"></span></p></div><div class="profile-stats"><div class="stat-item"><strong><span id="stat-total-entries">0</span></strong> Entries</div><div class="stat-item"><strong><span id="stat-net-worth">₹0</span></strong> Net Worth</div></div></div></div></div>
    <div class="card"><h3>🔐 Account</h3><div class="settings-group"><div class="settings-row"><label>Google Drive</label><div id="drive-status" class="status-indicator disconnected"><span>Disconnected</span></div></div><button id="google-disconnect-btn" class="btn btn-secondary hidden">Disconnect</button></div></div>
    <div class="card"><h3>🎨 Appearance</h3><div class="settings-group"><div class="settings-row"><label for="theme-select">Theme</label><select id="theme-select"><option value="auto">System</option><option value="light">Light</option><option value="dark">Dark</option></select></div></div><div class="settings-group"><div class="settings-row"><label for="currency-select">Currency</label><select id="currency-select"><option value="INR">INR (₹)</option><option value="USD">USD ($)</option><option value="EUR">EUR (€)</option><option value="GBP">GBP (£)</option></select></div></div><div class="settings-group"><div class="settings-row"><label for="date-format-select">Date Format</label><select id="date-format-select"><option value="local">Local Default</option><option value="DD/MM/YYYY">DD/MM/YYYY</option><option value="MM/DD/YYYY">MM/DD/YYYY</option></select></div></div></div>
     <div class="card"><h3>💾 Data Management</h3><div class="settings-group"><div class="settings-row"><p>Last cloud backup: <span id="last-backup-status">Never</span></p></div><button id="backup-now-btn" class="btn btn-secondary" disabled>Backup Now</button><button id="restore-data-btn" class="btn btn-secondary" disabled>Restore from Drive</button></div><div class="settings-group"><button id="export-json" class="btn btn-secondary">Export JSON</button><button id="export-csv" class="btn btn-secondary">Export CSV</button></div><div class="settings-group"><label for="import-json" class="btn btn-secondary">Import JSON</label><input type="file" id="import-json" accept=".json" class="hidden"></div>
     <div class="settings-group"><div class="settings-row"><label for="auto-diary-toggle">Auto-log completed/missed tasks</label><input type="checkbox" id="auto-diary-toggle" checked></div></div>
     <div class="settings-group"><button id="clear-data-btn" class="btn btn-danger">Clear Local Data</button><button id="delete-account-btn" class="btn btn-danger" disabled>Delete All Cloud Data</button></div></div>
  </div>
</main>
...
const DRIVE_FILE_NAME = 'alertjar_data.json';
const CONFIG = {
        ...
        newWorker.addEventListener('statechange', () => {
          if (newWorker.state === 'installed') {
            if (navigator.serviceWorker.controller) {
              ...
              showToast('New version available!', 'info', 0, action);
            } else {
...
const findEntry = (id) => dbRequest('entries', 'readonly', 'get', id);
...
function parseAmount(text) {
  ...
  const match = cleaned.match(/\b\d+(\.\d+)?k?\b/i);
  if (!match) return null;
  let num = parseFloat(match[0].replace(/\s/g, ''));
...
async function refreshDiary() {
    ...
    const allEntries = await getEntries({type: 'diary'});
    renderDiaryEntries(entries);
    renderDiaryTags(allEntries);
}
...
function renderTimeBlockedTasks(todos) {
    ...
    todos.sort((a,b) => a.todoMeta.from.localeCompare(b.todoMeta.from)).forEach(todo => {
        ...
        if (todo.todoMeta.status === 'done') tr.classList.add('done');
        tr.innerHTML = `
            ...
            <td class="check-cell"><button data-action="toggle-todo" aria-label="Toggle task: ${sanitize(todo.text)}"><svg><use href="#icon-check"/></svg></button></td>
        `;
...
async function checkMissedTasks() {
    try {
        ...
        const todos = await getEntries({ type: 'todo', date: getLocalDate() });
        const toUpdate = [], diaryEntries = [];
        const autoLogEnabled = document.getElementById('auto-diary-toggle').checked;

        for (const todo of todos) {
            if (todo.todoMeta?.isTimeBlocked && todo.todoMeta.status === 'pending' && todo.todoMeta.to < currentTime) {
...
async function toggleTodo(id) {
    const todo = await findEntry(id);
    if (!todo || !todo.todoMeta?.isTimeBlocked) { return showToast('Task not found.', 'error'); }
    ...
    if (todo.todoMeta.status === 'pending') {
        ...
        if (autoLogEnabled) {
            await saveEntry({ 
                id: uuid(), type: 'diary', date: getLocalDate(), timestamp: Date.now(), 
                text: `Completed Task (scheduled ${todo.todoMeta.from}-${todo.todoMeta.to}): ${todo.text}`, 
                tags: ['todo-done'], source: 'auto-completed', relatedTodoId: todo.id 
            });
        }
        showToast('Task completed!', 'success');
    } else if (todo.todoMeta.status === 'done') {
        await saveEntry({ ...todo, todoMeta: { ...todo.todoMeta, status: 'pending' } });
        if (autoLogEnabled) {
            const entries = await getRawEntries();
            const relatedDiaryEntry = entries.find(e => e.source === 'auto-completed' && e.relatedTodoId === todo.id);
            if (relatedDiaryEntry) {
                await deleteEntry(relatedDiaryEntry.id);
            }
        }
...
async function renderCalendar() {
  ...
  for (let i = 1; i <= totalCells; i++) {
    ...
    if (dayNum > 0 && dayNum <= daysInMonth) {
      ...
      grid.insertAdjacentHTML('beforeend', `<div role="gridcell" tabindex="0" class="calendar-day ${dateStr === getLocalDate() ? 'today' : ''}" data-date="${dateStr}" aria-label="${dayLabel}">${dayNum}${indicators ? `<div class="calendar-indicators">${indicators}</div>` : ''}</div>`);
    } else {
      grid.insertAdjacentHTML('beforeend', `<div role="gridcell" class="calendar-day other-month"></div>`);
    }
  }
}
...
document.getElementById('import-json').onchange = (e) => {
  ...
  reader.onload = async (event) => {
    try {
      ...
      if(!confirm(`WARNING: This will permanently ERASE all current data in the app and replace it with the file's content. This action CANNOT be undone. Proceed with import?`)) return;
...
const applyTheme = (theme) => {
  ...
  try { localStorage.setItem('themeChoice', theme); } catch(e) { console.warn('Could not save theme to localStorage.'); }
...
document.addEventListener('click', (e) => {
  ...
  const calendarDay = target.closest('.calendar-day[role="gridcell"][data-date]');
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.target.matches('.calendar-day[role="gridcell"]') && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); showDayEntries(e.target.dataset.date); }
...
async function findDriveFileId() {
  try {
    ...
    if(err.status === 401) showToast('Google session expired. Please disconnect and reconnect.', 'error');
    else console.error('Error finding file:', err);
    return null;
  }
}

async function backupToDrive(btnEl) {
  btnEl.disabled = true; btnEl.textContent = 'Backing up...';
  try {
    const entries = await getRawEntries();
    const fileContent = JSON.stringify({ version: APP_VERSION, timestamp: Date.now(), entries });
    ...
    document.getElementById('last-backup-status').textContent = new Date().toLocaleString();
    showToast('Backup successful!', 'success');
  } catch (error) {
    if(error.status === 401) showToast('Google session expired. Please disconnect and reconnect.', 'error');
    else console.error('Backup failed:', error);
    showToast('Backup failed.', 'error');
  } finally {
    btnEl.disabled = false; btnEl.textContent = 'Backup Now';
  }
}

async function restoreFromDrive(btnEl) {
  if (!confirm("This will overwrite all local data with your Google Drive backup. This cannot be undone. Are you sure?")) return;
  btnEl.disabled = true; btnEl.textContent = 'Restoring...';
  try {
    const fileId = await findDriveFileId();
    if (!fileId) {
      showToast('No backup file found in Google Drive.', 'info');
      return;
    }
    const res = await gapi.client.drive.files.get({ fileId: fileId, alt: 'media' });
    const data = JSON.parse(res.body);
    if (!data.entries || !validateImportedData(data.entries)) throw new Error("Invalid backup file format.");
    ...
    showToast(`Restored ${data.entries.length} entries successfully.`, 'success');
    await refreshAll();
  } catch (error) {
    if(error.status === 401) showToast('Google session expired. Please disconnect and reconnect.', 'error');
    else console.error('Restore failed:', error);
    showToast('Restore failed. The backup may be corrupt.', 'error');
  } finally {
    btnEl.disabled = false; btnEl.textContent = 'Restore from Drive';
  }
}

let googleInitCalled = {}; // Use an object to track individual failures
function disableGoogleFeatures(type) {
    if (googleInitCalled[type]) return;
    googleInitCalled[type] = true;
    showToast(`Google service (${type}) is unavailable.`, 'error');
    // Check if both have failed before disabling everything
    if(googleInitCalled['gapi'] && googleInitCalled['gis']) {
...
function gapiLoaded() {
    if (typeof gapi === 'undefined' || !gapi.load) {
        ...
        return disableGoogleFeatures('gapi');
    }
...
function scheduleMissedTaskCheck() {
    checkMissedTasks().finally(() => {
        setTimeout(scheduleMissedTaskCheck, 60 * 1000);
    });
}
...
async function init() {
  try {
    ...
    document.getElementById('backup-now-btn').addEventListener('click', (e) => backupToDrive(e.target));
    document.getElementById('restore-data-btn').addEventListener('click', (e) => restoreFromDrive(e.target));
    
    const autoLogToggle = document.getElementById('auto-diary-toggle');
    const savedAutoLog = localStorage.getItem('autoLogTasks');
    if (savedAutoLog) autoLogToggle.checked = savedAutoLog === 'true';
    autoLogToggle.addEventListener('change', (e) => localStorage.setItem('autoLogTasks', e.target.checked));
  ...
  try{
    const savedTheme = localStorage.getItem('themeChoice') || 'auto';
